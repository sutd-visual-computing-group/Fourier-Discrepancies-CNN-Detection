...
# and use a KNN classifier to perform binary classification on real vs fake. 
# We use k=5 as original work
...
def train_one_knn_and_test( dataset,
                            adversarial_mode,
                            num_samples, split):
    """
    Train 1 KNN classifier (k=5) and test
    ...
    # Defind a kNN classsifer with 5 neighbours
    ...
    neigh.fit(scaler.transform(X_train), Y_train)

    # Get training accuracy on real and GAN data seperately
    train_acc_real = neigh.score(scaler.transform(X_train[:tr_real_data_break_point]), Y_train[:tr_real_data_break_point])
    train_acc_fake = neigh.score(scaler.transform(X_train[tr_real_data_break_point:]), Y_train[tr_real_data_break_point:])
    ...
    test_acc_real = neigh.score(scaler.transform(X_test_real), Y_test_real)
    test_acc_fake = neigh.score(scaler.transform(X_test_gan), Y_test_gan)
    ...
    for i in setups:
        ...
        acc = neigh.score(scaler.transform(X_test), Y_test)
...
def train_multiple_knns(model_str, num_experiments, num_samples, split):
    ...
    for exp in tqdm(range(num_experiments)):
        final_accs = train_one_knn_and_test('celeba', model_str, num_samples, split)
        df['exp{}'.format(exp)] = final_accs
        
    df.to_csv('csv/{}_knn_results_{}_{}.csv'.format(model_str, num_samples, split), index=None)
    return
...
def main():
    ...
    train_multiple_knns(args.model_str, args.num_exp, args.num_samples, args.split)
